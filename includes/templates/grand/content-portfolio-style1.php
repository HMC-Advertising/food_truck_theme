...
<article <?php post_class($article_port_class); ?>> <!-- post class -->
    <?php
    $title_before = '<h4 class="df-post-title">';
    $title_after = '</h4>';
    if (!is_single()) {
    ...
    <div class="entry df-portfolio-content"> <!-- post class -->
        ...
        if (has_post_thumbnail() && $preview_style == "featured") {
                ...
                if ($page_metabox_image_resize_port == 'custom') {
                    $img_url = esc_url(wp_get_attachment_url(get_post_thumbnail_id(), 'full')); //get full URL to image (use "large" or "medium" if the images too big)
                ...
                echo "<div class='border-port-style1'></div>";
                echo '<div class="clear"></div>';
